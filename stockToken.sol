...
contract stockToken is ERC20Snapshot, ReentrancyGuard {
      
      IPyth pyth;

    // Defineix la quantitat total de tokens
    // Es multiplica per 10*18 per a definir decimals de la mateixa manera que Ether i altres.
    uint256 public constant TOTAL_TOKENS = 10 * 10**18;
    ...
    constructor() ERC20("Mock Stock token", "TOKENSIMU") {
        ...
        // Crea el suministre inicial i l'assigna al propi contracte
 ...
 // }
  ...
  function claimDividends(uint256 snapshotId) external {
    ...
    uint256 payment = (userBalance * dividends[snapshotId].totalAmount) / totalSupply();
